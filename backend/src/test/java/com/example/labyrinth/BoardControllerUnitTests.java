...
public class BoardControllerUnitTests {
    ...
    void rotateSpare_returnsUpdatedTile() throws Exception {
        ...
        when(boardService.rotateSpareClockwise(anyInt())).thenReturn(spare);

        mvc.perform(post("/api/boards/spare/rotate")).andExpect(status().isOk())
            .andExpect(jsonPath("$.exitWest", is(true)));
    }

    @Test
    void rotateSpare_noSpare_returnsBadRequest() throws Exception {
        when(boardService.rotateSpareClockwise(anyInt())).thenThrow(new IllegalStateException("no spare"));
        mvc.perform(post("/api/boards/spare/rotate")).andExpect(status().isBadRequest());
    }
    ...
    void rotateSpare_ccw_returnsUpdatedTile() throws Exception {
        ...
        when(boardService.rotateSpareCounterclockwise(anyInt())).thenReturn(spare);

        mvc.perform(post("/api/boards/spare/rotate?direction=ccw")).andExpect(status().isOk())
